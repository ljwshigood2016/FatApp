...
public class CircleProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        LinearGradient lg =new LinearGradient(0,0,100,100,0xFFbfbfbf,0xFFfbfbfb,Shader.TileMode.MIRROR);  
        
       // innerCirclePaint.setColor(Color.GRAY);
        innerCirclePaint.setAntiAlias(true);
        innerCirclePaint.setShader(lg);
        
        float innerCircleRadius = (getWidth() - roundWidth*2) / 2f;
        ...
        if(textIsDisplayable && percent != 0 && style == STROKE){
           // canvas.drawText(percent +"\n"+"ok", centre - textWidth / 2, centre + textSize/2, paint); //画出进度百分比
        }
        ...
        switch (style) {
            case STROKE:{
                ...
                canvas.drawArc(oval, 98, 360 * progress / max, false, paint1);  //根据进度画圆弧
                break;
            }
            case FILL:{
